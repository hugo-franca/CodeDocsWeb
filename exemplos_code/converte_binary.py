...
nomeArquivoBase = "BinaryCollision.sim"
nomeNovoArquivo = "html_Binary.txt"
...
def formataLinha(linha):
	...
	elif( indiceLinhaAtual==17 ):
		# REGIAO_LIVRE tipo=COLISAO_DUAS_GOTAS centro_base=[0.0 0.0] raios=[0.5 0.5] B=0.3 distancia=0.5 merge_time=0.0
		stringParse = "REGIAO_LIVRE tipo={} centro_base=[{}] raios=[{}] B={} distancia={} merge_time={}"
		resultado = parse(stringParse, linha)

		tipo = tooltipString(resultado[0], dict_tooltips['regiao_livre_tipo_binary'])
		centro = tooltipString(resultado[1], dict_tooltips['regiao_binary_center'])
		raios = tooltipString(resultado[2], dict_tooltips['regiao_binary_raios'])
		B = tooltipString(resultado[3], dict_tooltips['regiao_binary_B'])
		distancia = tooltipString(resultado[4], dict_tooltips['regiao_binary_distancia'])
		merge_time = tooltipString(resultado[5], dict_tooltips['regiao_binary_mergetime'])
		
		output_total = stringParse.format(tipo, centro, raios, B, distancia, merge_time)
	else:
		output_total = linha
